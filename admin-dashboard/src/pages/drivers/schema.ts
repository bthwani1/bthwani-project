...
export const createDriverSchema = z.object({
  ...
  // إذا كان نوع السائق جوكر، يجب أن تكون تواريخ الجوكر محددة
  if (data.driverType === "joker") {
    if (!data.jokerFrom || !data.jokerTo) {
      return false;
    }
...
// Schema لتحديث بيانات السائق (بدون كلمة المرور)
export const updateDriverSchema = createDriverSchema.omit({ password: true }).extend({
  // جعل بعض الحقول اختيارية للتعديل
