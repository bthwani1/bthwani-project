...
import { getAmaniList, updateAmaniStatus, deleteAmani, type AmaniItem } from '../../../api/amani';
import { AmaniStatus, AmaniStatusLabels, AmaniStatusColors } from '../../../types/amani';
...
const AmaniListPage: React.FC = () => {
  ...
  const fetchAmaniItems = useCallback(async (cursor?: string, append: boolean = false) => {
    try {
      ...
      setAmaniItems(prev => append ? [...prev, ...newItems] : newItems);
  ...
  const handleStatusUpdate = async (amani: AmaniItem, newStatus: AmaniStatus) => {
    try {
      await updateAmaniStatus(amani._id, newStatus);
      setSnackbar({
        open: true,
