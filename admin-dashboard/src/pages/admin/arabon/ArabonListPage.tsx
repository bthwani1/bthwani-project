...
import { getArabonList, updateArabonStatus, deleteArabon, type ArabonItem } from '../../../api/arabon';
import { ArabonStatus, ArabonStatusLabels, ArabonStatusColors } from '../../../types/arabon';
...
const ArabonListPage: React.FC = () => {
  ...
  const fetchArabonItems = useCallback(async (cursor?: string, append: boolean = false) => {
    try {
      ...
      setArabonItems(prev => append ? [...prev, ...newItems] : newItems);
  ...
  const handleStatusUpdate = async (arabon: ArabonItem, newStatus: ArabonStatus) => {
    try {
      await updateArabonStatus(arabon._id, newStatus);
      setSnackbar({
        open: true,
