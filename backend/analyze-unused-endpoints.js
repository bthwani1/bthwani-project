...
function searchEndpointUsage(endpoint, searchDir) {
  try {
    ...
    // 1. Direct path match (e.g., "users/me")
    // 2. axios.get/post/patch/delete with the path
    // 3. fetch with the path
    // 4. Template literals with the path
    const patterns = [
      cleanEndpoint,
      `["']${cleanEndpoint}["']`,
    ...
    for (const pattern of patterns) {
      try {
        const cmd = `cd "${searchDir}" && find . -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" | xargs grep -l "${pattern}" 2>/dev/null || true`;
...
function analyzeUnusedEndpoints() {
  ...
  allRoutes.forEach(route => {
    ...
    for (const frontendDir of frontendDirs) {
      if (fs.existsSync(path.join(__dirname, frontendDir))) {
        if (searchEndpointUsage(route.path, path.join(__dirname, frontendDir))) {
          isUsed = true;
          break;
        }
      }
