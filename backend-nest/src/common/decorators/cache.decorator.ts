...
function generateCacheKey(methodName: string, args: any[]): string {
  ...
  // Check for custom cache key templates
  const keyTemplates = Reflect.getOwnMetadata('cache-keys', args.callee) || [];

  if (keyTemplates.length > 0) {
    // Use custom key template
    key = keyTemplates.map((template: string, index: number) => {
      if (template && args[index] !== undefined) {
        return template.replace(/{(\w+)}/g, (match, param) => {
          // Simple parameter replacement
          return args[index]?.[param] || args[index]?.toString() || 'undefined';
        });
      }
